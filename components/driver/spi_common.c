...
esp_err_t spicommon_bus_initialize_io(spi_host_device_t host, const spi_bus_config_t *bus_config, int dma_chan, uint32_t flags, uint32_t *flags_o)
{
    ...
    if (bus_config->sclk_io_num >= 0)
    {
        ...
        if (bus_config->sclk_io_num != spi_periph_signal[host].spiclk_iomux_pin)
            use_iomux = false;
    }
    else
    {
    ...
    if (bus_config->quadwp_io_num >= 0)
    {
        ...
        if (bus_config->quadwp_io_num != spi_periph_signal[host].spiwp_iomux_pin)
            use_iomux = false;
    }
    else
    {
    ...
    if (bus_config->quadhd_io_num >= 0)
    {
        ...
        if (bus_config->quadhd_io_num != spi_periph_signal[host].spihd_iomux_pin)
            use_iomux = false;
    }
    else
    {
    ...
    if (bus_config->mosi_io_num >= 0)
    {
        ...
        if (bus_config->mosi_io_num != spi_periph_signal[host].spid_iomux_pin)
            use_iomux = false;
    }
    else
    {
    ...
    if (bus_config->miso_io_num >= 0)
    {
        ...
        if (bus_config->miso_io_num != spi_periph_signal[host].spiq_iomux_pin)
            use_iomux = false;
    }
    else
    {
...
{
    if (REG_GET_FIELD(GPIO_PIN_MUX_REG[spi_periph_signal[host].spid_iomux_pin], MCU_SEL) == 1)
        PIN_FUNC_SELECT(GPIO_PIN_MUX_REG[spi_periph_signal[host].spid_iomux_pin], PIN_FUNC_GPIO);
    if (REG_GET_FIELD(GPIO_PIN_MUX_REG[spi_periph_signal[host].spiq_iomux_pin], MCU_SEL) == 1)
        PIN_FUNC_SELECT(GPIO_PIN_MUX_REG[spi_periph_signal[host].spiq_iomux_pin], PIN_FUNC_GPIO);
    if (REG_GET_FIELD(GPIO_PIN_MUX_REG[spi_periph_signal[host].spiclk_iomux_pin], MCU_SEL) == 1)
        PIN_FUNC_SELECT(GPIO_PIN_MUX_REG[spi_periph_signal[host].spiclk_iomux_pin], PIN_FUNC_GPIO);
    if (REG_GET_FIELD(GPIO_PIN_MUX_REG[spi_periph_signal[host].spiwp_iomux_pin], MCU_SEL) == 1)
        PIN_FUNC_SELECT(GPIO_PIN_MUX_REG[spi_periph_signal[host].spiwp_iomux_pin], PIN_FUNC_GPIO);
    if (REG_GET_FIELD(GPIO_PIN_MUX_REG[spi_periph_signal[host].spihd_iomux_pin], MCU_SEL) == 1)
        PIN_FUNC_SELECT(GPIO_PIN_MUX_REG[spi_periph_signal[host].spihd_iomux_pin], PIN_FUNC_GPIO);
...
void spicommon_cs_initialize(spi_host_device_t host, int cs_io_num, int cs_num, int force_gpio_matrix)
{
    ...
    else
    {
        ...
        if (cs_num == 0)
            gpio_matrix_in(cs_io_num, spi_periph_signal[host].spics_in, false);
