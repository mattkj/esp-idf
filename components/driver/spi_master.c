...
esp_err_t spi_bus_add_device(spi_host_device_t host, const spi_device_interface_config_t *dev_config, spi_device_handle_t *handle)
{
    ...
    for (freecs = 0; freecs < NO_CS; freecs++)
    {
        ...
        void *null = NULL;
        if (atomic_compare_exchange_strong(&spihost[host]->device[freecs], &null, (spi_device_t *)1))
            break;
    }
    ...
    SPI_CHECK(dev_config->cs_ena_pretrans <= 1 || (dev_config->address_bits == 0 && dev_config->command_bits == 0) ||
              ...
              "In full-duplex mode, only support cs pretrans delay = 1 and without address_bits and command_bits", ESP_ERR_INVALID_ARG);
...
esp_err_t spi_bus_remove_device(spi_device_handle_t handle)
{
    ...
    for (x = 0; x < NO_CS; x++)
    {
        if (atomic_load(&handle->host->device[x]) == handle)
        {
            ...
            if (x == handle->host->prev_cs)
                handle->host->prev_cs = NO_CS;
        }
    }
    free(handle);
    return ESP_OK;
...
static void SPI_MASTER_ISR_ATTR spi_setup_device(spi_host_t *host, int dev_id)
{
    ...
    if (host->hw->ctrl2.hold_time == 0)
        host->hw->ctrl2.hold_time = 1;
...
static SPI_MASTER_ISR_ATTR esp_err_t check_trans_valid(spi_device_handle_t handle, spi_transaction_t *trans_desc)
{
    ...
    SPI_CHECK(!(handle->cfg.flags & SPI_DEVICE_HALFDUPLEX) || host->dma_chan == 0 || !(trans_desc->flags & SPI_TRANS_USE_RXDATA || trans_desc->rx_buffer != NULL) || !(trans_desc->flags & SPI_TRANS_USE_TXDATA || trans_desc->tx_buffer != NULL), "SPI half duplex mode does not support using DMA with both MOSI and MISO phases.", ESP_ERR_INVALID_ARG);
